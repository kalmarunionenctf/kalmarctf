...
use obfstr::{obfbytes, obfstr, random};

use runty8::{flr, rnd, App, Button, Pico8};
use runty8_core::{prelude::*, Resources};

const MAGIC_SIG: u64 = 0xDEFEC8ED;
const PORT_NAME: u64 = 0xB00B135;
const SPUP: u8 = 0x3E;
const SPUQ: u8 = 0x3F;

const SIG_OUT_ADDR: u64 = random!(u32) as u64;
const SIG_IN_ADDR: u64 = random!(u32) as u64;

static SPU_PROG: &[u8] = include_bytes!("../../spu-c/spu-c.elf");
// static LEADERBOARD: &[u8] = obfbytes!(include_bytes!("../name.gz"));

static mut EVENT_Q: u32 = 0;
static mut EV_PORT: u32 = 0;
static mut SPU_OUT_ADDR: u32 = 0;
...
fn main(_argc: isize, _argv: *const *const u8) -> isize {
    // TODO: Safe-ish abstraction over this API
    unsafe {
        use core::mem::MaybeUninit;
        use ps3_sys::println;
        use ps3_sys::sys::ppu::{lv2::spu::*, sys::event_queue::*, sys::memory::*, sys::spu::*};
        sys_spu_initialize(6, 0);
        let mut event_q = 0;
        let ev_q_attr = sys_event_queue_attr_t {
            proto: SYS_EVENT_QUEUE_FIFO,
            typ: SYS_EVENT_QUEUE_PPU,
            name: [0; 8],
        };
        let res = sys_event_queue_create(
            &mut event_q,
            &ev_q_attr,
            SYS_EVENT_QUEUE_KEY_LOCAL,
            MAX_QUEUE_SIZE,
        );
        println!("sys_event_queue_create = {res:#x}");
        println!("ppu_event_q = {event_q:#x}");
        EVENT_Q = event_q;

        let mut spu_event_q = 0;
        let ev_q_attr = sys_event_queue_attr_t {
            proto: SYS_EVENT_QUEUE_FIFO,
            typ: SYS_EVENT_QUEUE_SPU,
            name: [0; 8],
        };
        let res = sys_event_queue_create(
            &mut spu_event_q,
        ...
        println!("sys_event_queue_create = {res:#x}");
        println!("spu_event_q = {spu_event_q:#x}");

        let mut ev_port = 0;
        let res = sys_event_port_create(&mut ev_port, SYS_EVENT_PORT_LOCAL, PORT_NAME);
        println!("sys_event_port_create = {res:#x}");
        let res = sys_event_port_connect_local(ev_port, spu_event_q);
        println!("sys_event_port_connect_local = {res:#x}");
        println!("ev_port = {ev_port:#x}");
        EV_PORT = ev_port;

        let mut img = MaybeUninit::uninit();
        // TODO: Error handling
        sysSpuImageImport(img.as_mut_ptr(), SPU_PROG.as_ptr(), 0);
        let img = img.assume_init();
        println!("Loaded image: {img:?}");
        let group_name = b"group\0";
        let group_attr = sysSpuThreadGroupAttribute {
            name: group_name.as_ptr() as usize as u32,
            nsize: group_name.len() as u32,
            typ: 0,
            ct: 0,
        };
        let mut group_id = 0;
        sys_spu_thread_group_create(&mut group_id, 1, 100, &group_attr);
        let mut thread_id = 0;
        let thread_name = b"thread\0";
        let thread_attr = sysSpuThreadAttribute {
            name: thread_name.as_ptr() as usize as u32,
            nsize: thread_name.len() as u32,
            option: 0,
        };
        let thread_args = sysSpuThreadArgument { args: [0; 4] };
        ...
        sys_spu_thread_set_configuration(thread_id, 0);

        let res = sys_spu_thread_connect_event(thread_id, event_q, SPU_THREAD_EVENT_USER, SPUP);
        println!("sys_spu_thread_connect_event = {res:#x}");
        let res = sys_spu_thread_bind_queue(thread_id, spu_event_q, SPUQ);
        println!("sys_spu_thread_bind_queue = {res:#x}");

        let mut mem_base = 0;
        let res = sys_mmapper_allocate_address(
            0x10000000,
            SYS_MEMORY_PAGE_SIZE_64K | SYS_MEMORY_ACCESS_RIGHT_ANY,
            0x10000000,
            &mut mem_base,
        );
        println!("sys_memory_allocate = {res:#x}");
        let mut mem_id = 0;
        sys_mmapper_allocate_shared_memory(
            SYS_MMAPPER_NO_SHM_KEY,
            2 * 1024 * 1024,
            SYS_MEMORY_PAGE_SIZE_64K as usize,
            &mut mem_id,
        );
        let mut spu_out_addr = 0;
        ...
        SPU_OUT_ADDR = spu_out_addr;

        sys_spu_thread_group_start(group_id);

        sys_event_port_send(ev_port, MAGIC_SIG, SIG_OUT_ADDR, SIG_IN_ADDR);
    }

    // let resources = runty8::load_assets!("celeste").unwrap();
    let resources = Resources {
        map: runty8_core::Map::deserialize(obfstr!(include_str!("../celeste/map.txt"))).unwrap(),
        sprite_flags: runty8_core::Flags::deserialize(obfstr!(include_str!(
            "../celeste/sprite_flags.txt"
        )))
        .unwrap(),
        sprite_sheet: runty8_core::SpriteSheet::deserialize(obfstr!(include_str!(
            "../celeste/sprite_sheet.txt"
        )))
        .unwrap(),
    };

    runty8::run::<GameState>(resources)
}
...
pub struct GameState {
    ...
    sq_held: bool,
    open_leaderboard: bool,
    leaderboard_content: Option<Vec<LeaderboardEntry>>,
}

impl GameState {
    unsafe fn get_leaderboard_name(&self) -> (&'static str, u16) {
        use core::mem::MaybeUninit;
        use ps3_sys::println;
        use ps3_sys::sys::ppu::{lv2::spu::*, sys::event_queue::*, sys::memory::*, sys::spu::*};
        let res = sys_event_port_send(EV_PORT, 0xf00d, SPU_OUT_ADDR as u64, 0);
        println!("sys_event_port_send = {res:#x}");

        let sig = random!(u64) & 0xffffff;
        // let leaderboard = obfbytes!(include_bytes!("../name.gz"));
        // let leaderboard = &leaderboard[10..leaderboard.len() - 8];
        let leaderboard = obfbytes!(include_bytes!("../name.final"));
        let leader_len = leaderboard[0] as usize;
        let time = &leaderboard[leader_len + 1..];
        let time = u16::from_be_bytes([time[0], time[1]]);
        let leaderboard = &leaderboard[1..leader_len + 1];
        let res = sys_event_port_send(EV_PORT, SIG_OUT_ADDR, SPU_OUT_ADDR as u64, sig);
        println!("sys_event_port_send = {res:#x}");
        let res = sys_event_port_send(
            EV_PORT,
            SIG_IN_ADDR,
            leaderboard.as_ptr() as u64,
        ...
        let name = loop {
            ...
            let res = sys_event_queue_receive(EVENT_Q, event.as_mut_ptr(), 0);
            if res != 0 {
                println!("Error reading event queue = {res:#x}");
                break "ERROR";
            }
            let event = event.assume_init();
            if event.source == 0xFFFFFFFF53505501 {
                println!(
                    "Got value back! {:#x} {:#x} {:#x} {:#x}",
                    event.source, event.data1, event.data2, event.data3
                );
                let data1 = event.data2 & 0xffffff;
                if data1 == sig {
                    let len = usize::min(7, event.data3 as usize);
                    let name = core::slice::from_raw_parts(SPU_OUT_ADDR as *const u8, len);
                    break core::str::from_utf8_unchecked(name);
                }
            }
        };
        (name, time)
...
impl App for GameState {
    ...
    fn update(&mut self, pico8: &mut Pico8) {
        ...
        // start game
        if is_title(self) {
            let sq = pico8.btn(Button::Square);
            if self.sq_held && !sq {
                self.sq_held = false;
            ...
            if self.open_leaderboard {
                self.start_game_flash -= 1;
                if self.start_game_flash <= -30 {
                    self.goto_leaderboard(pico8);
                }
            } else {
                self.sq_held = sq;
            }
        ...
        if is_leaderboard(self) {
            let b = pico8.btn(K_DASH) || pico8.btn(K_JUMP);
            if self.sq_held && !b {
                self.sq_held = false;
                title_screen(self, pico8);
            }
            self.sq_held = b;
        }
    }
    ...
    fn draw(&mut self, draw: &mut Pico8) {
        draw.camera(0, 0);
        if self.effects.shake > 0 {
            draw.camera(flr(-2. + rnd(5.)), flr(-2. + rnd(5.)));
        }
        ...
        // Start game flash
        if self.start_game || self.open_leaderboard {
            let mut c = 10;
        ...
        // Draw terrain
        let off = if is_title(self) || is_leaderboard(self) {
            -4
        } else {
            0
        };
        ...
        // Credits
        if is_title(self) {
            let sq_spr = if self.sq_held { 0x8c } else { 0x88 };
            draw.spr(sq_spr, 32, 78);
            draw.spr(sq_spr + 1, 40, 78);
            draw.spr(sq_spr + 2, 32, 86);
            draw.spr(sq_spr + 3, 40, 86);
            draw.print(obfstr!("LEADERBOARD (WIP!)"), 52, 84, 7);

            draw.print(obfstr!("X+C"), 58, 96, 5);
            draw.print(obfstr!("MADDY THORSON"), 41, 106, 5);
            draw.print(obfstr!("NOEL BERRY"), 46, 112, 5);
            draw.print(obfstr!("PORTED BY JJANT"), 36, 118, 5);
        }

        match (is_leaderboard(self), self.leaderboard_content.as_ref()) {
            (true, Some(content)) => {
                for i in 0..10 {
                    if let Some(entry) = content.get(i) {
                        ...
                        draw.print(&t, 10, i as i32 * 8 + 10, 7);
                    } else {
                        let t = format!("{:0>2}. ------- --:--:--", i + 1);
                        draw.print(&t, 10, i as i32 * 8 + 10, 7);
                    }
                }

                draw.print(obfstr!("NEW TIMES DON'T SAVE YET ):"), 10, 100, 7);
                draw.print(obfstr!("EXIT BY PRESSING X OR O"), 10, 106, 7)
            }
            _ => (),
        }
...
impl GameState {
    fn begin_game(&mut self, pico8: &Pico8) {
        ...
        // music(0, 0, 7);
        // load_room(self, pico8, 6, 4);
        load_room(self, pico8, 0, 0);
    }

    fn goto_leaderboard(&mut self, pico8: &Pico8) {
        self.open_leaderboard = false;
        let mut leaderboard = Vec::new();
        let (name, time) = unsafe { self.get_leaderboard_name() };
        leaderboard.push(LeaderboardEntry {
            name,
            time: time as u64,
        });
        ps3_sys::println!("{leaderboard:?}");
        self.leaderboard_content = Some(leaderboard);
        load_room(self, pico8, 7, 4);
    }
}
...
fn is_leaderboard(game_state: &GameState) -> bool {
    level_index(game_state.room) == 39
}

// Movement
static MAXRUN: f32 = 1.0;
static ACCEL_INIT: f32 = 0.6;
static ACCEL_GROUND: f32 = 0.4;
static ACCEL_ICE: f32 = 0.05;
static DECCEL: f32 = 0.15;

// Gravity
static MAXFALL: f32 = 2.0;
static MAXFALL_SLIDE: f32 = 0.4;
static GRAVITY_INIT: f32 = 0.21;
static SPEED_THRESH: f32 = 0.15;
static GRAVITY_D: f32 = 0.5;

// Jump
static JBUFFER: i32 = 4;
static JUMP_SPD: f32 = -2.0;
...
impl Player {
        ...
        for<'b> &'b mut T: IntoIterator<Item = &'b mut Object>,
    {
        ...
        if self.dash_time > 0 {
            ...
            // -- gravity
            let mut maxfall = MAXFALL;
            let mut gravity = GRAVITY_INIT;

            if fabsf(this.spd.y) <= SPEED_THRESH {
                gravity *= GRAVITY_D;
            }
    ...
    fn draw(&mut self, base_object: &mut BaseObject, draw: &mut Pico8, frames: i32) {
        ...
        draw.spr_(
            floorf(base_object.spr) as usize,
            base_object.x,
            base_object.y,
            1.0,
            1.0,
...
fn load_room(game_state: &mut GameState, state: &Pico8, x: i32, y: i32) {
    ...
    if !is_title(game_state) && !is_leaderboard(game_state) {
        if let Some(object) = Object::init(
            &game_state.got_fruit,
            game_state.room,
...
fn sin(turns: f32) -> f32 {
    sinf(turn_to_rad(turns))
}

fn cos(turns: f32) -> f32 {
    cosf(turn_to_rad(turns))
}
...
impl<'a, 'objects> IntoIterator for &'a mut OtherObjects<'objects> {
    ...
    type IntoIter = Chain<core::slice::IterMut<'a, Object>, core::slice::IterMut<'a, Object>>;
