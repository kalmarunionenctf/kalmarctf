...
//TODO: Use non-debug values for actual CTF
//#define DEBUG
#ifdef DEBUG
#define TIMEOUT 300
...
// At first we xor with random stuff, and at then end with the flag if we have odd steps.
// This forces the user to end with a bunch of odd steps, so in the easy version of the chal
// only even and at the end odd will work.
#define SEQUENCELEN (SEQUENCELEN_RANDOM + FLAGLEN)
// For a prescribed sequence of odd/even of length there is exactly one number < 2**n whose
// Collatz sequence has those steps.
// So the start number should have the same number of bits as the sequence length.
// Overflows will not be allowed, so the user will have to ensure that there are enough even
// steps, particularly at the start.
...
unsigned char step(unsigned char* value, unsigned int length)
{
  ...
  // the bits in a byte are ordered in big endian order
  ...
  if(result)
  {
    ...
    // tmp is the carry from previous byte
    unsigned long tmp = 1;
...
//#endif
  ...
  // will want the odd/even steps of the Collatz sequence starting with that number to be.
  // The required number might have about as many bits as the length of the sequence,
  // but the values reached in the sequence are not allowed to have more bits.
  // As odd steps multiply by roughly 3/2 and even ones by 1/2 it must be possible to
  // construct a start number with a sequence with the required other properties such that
  // there are sufficiently many even steps.
  // We will require that each of the random bytes will fall on even steps maximum FLAGLENGTH
  // times. The condition below means that each random byte will occur 2*FLAGLENGTH times,
  // so the requirement will mean that up to roughly half of the steps are allowed to be
  // even, which means up to some constraints at the start the values should fall so that
  // the limited width available for the values should not pose a problem.
  ...
  // Get a random seed for the random number generator and communicate it to the user
  unsigned int seed;
  ...
  //Assert some things about the flag so that the players know
  //what to expect
  ...
  // flag_xor_count[flag_index] will store for each rand_index
  // whether that random byte was xored an odd number of times into this flag byte 
  ...
  #ifdef REVENGE
  // We require that each byte of the encrypted flag has been xored an odd number of times by
  // some random byte
  ...
  // We also require that each random byte has been used at least number of occurences - FLAGLEN times
